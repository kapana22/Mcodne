...
const sBookingId = await idPage.evaluate(() => document.querySelector('a[href^="/me/bookings/"]')?.getAttribute('href')?.split('/')[3]?.split(/[?#]/)[0] || null)
await idCtx.close()
...
const tBookingId = await tPage.evaluate(() => document.querySelector('a[href^="/work/bookings/"]')?.getAttribute('href')?.split('/')[3]?.split(/[?#]/)[0] || null)
await tCtx.close()
...
// student
await run('student-dash-cancel', 'student', '/me', ['button:has-text("გაუქმება")'])
if (sBookingId) {
  await run('student-detail-cancel', 'student', `/me/bookings/${sBookingId}`, ['button:has-text("გაუქმება")'])
  await run('student-detail-reschedule', 'student', `/me/bookings/${sBookingId}`, ['button:has-text("გადადება")'])
  await run('student-detail-review', 'student', `/me/bookings/${sBookingId}?review=1`, [])
}
...
// tutor
await run('tutor-dash-decline', 'tutor', '/work', ['button:has-text("უარყოფა")'])
if (tBookingId) {
  await run('tutor-detail-reschedule', 'tutor', `/work/bookings/${tBookingId}`, ['button:has-text("გადადება")'])
}
await run('tutor-schedule-addslot', 'tutor', '/work/schedule', ['button:has-text("სლოტ")'])
await run('tutor-schedule-template', 'tutor', '/work/schedule', ['button:has-text("შაბლონ")'])
await run('tutor-profile-visibility', 'tutor', '/work/profile', ['button:has-text("დამალვა"), button:has-text("გამოქვეყნება")'])
