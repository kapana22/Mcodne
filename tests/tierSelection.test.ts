 ...
 */
{
  ...
  check('headline price is the FLAGSHIP, not the cheapest tier',
    head.label === '₾80', head.label)
  check('headline duration is the flagship\'s real length',
    head.minutes === 60, String(head.minutes))
  check('the free intro never sets the headline price',
    primaryPriceLabel([INTRO, QUICK], 80, 30).label === '₾45')
  check('payload order does not matter',
    primaryPriceLabel([FULL, INTRO, QUICK], 80, 30).minutes === 60)
  ...
  check('an all-free flagship still reads „უფასო", never „₾0"',
    primaryPriceLabel([INTRO], 0, 15).label === 'უფასო')
  check('flagship duration always matches primaryServiceMin',
    primaryPriceLabel([INTRO, QUICK, FULL], 80, 30).minutes
      === primaryServiceMin([INTRO, QUICK, FULL], 30))
}
