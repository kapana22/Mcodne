...
// sees at /apply.
//
// WHY IT EXISTS: /apply used to call requireRole(['STUDENT']) in its layout, so
...
export async function ApplyMarketing() {
  ...
  // Both CTAs carry ?redirect=/apply so signing in lands the person back here —
  // and, being a STUDENT by then, straight into the form.
  const signup = '/signup?redirect=%2Fapply'
  const signin = '/signin?redirect=%2Fapply'
