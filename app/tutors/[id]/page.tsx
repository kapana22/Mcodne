...
export async function generateMetadata(
  ...
  if (tutor && tutor !== 'error') {
    ...
    // Avatars are stored as data: URIs, which absoluteAvatar (correctly) can't
    // turn into an absolute URL — so `image` was usually null and the whole
    // `images` key was omitted, leaving the profile with NO og:image at all.
    // socialMeta falls back to /og.png, so a shared expert link can never
    // unfurl as a blank card again. (An expert with a real hosted avatar still
