...
import { stepDotClass } from '@/components/StepIndicator'
        ...
        {STEPS.map(s => {
          ...
          return (
            <li key={s.id}>
              ...
              >
                {/* The same circle as the top strip — one source, components/StepIndicator. */}
                <span className={`relative z-10 w-9 h-9 shrink-0 rounded-full inline-flex items-center justify-center transition-all duration-fast ${
                  stepDotClass(isDone ? 'done' : isActive ? 'active' : 'todo')
                }`}>
