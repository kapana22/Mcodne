        ...
        <section className="mt-8">
          ...
          {experts === null ? (
            ...
            <div className="rounded-card border border-dashed border-ink-200 bg-ink-50/40 p-8 text-center">
              ...
              <Link href="/tutors" className="mt-4 inline-flex items-center gap-1.5 h-11 px-4 rounded-btn bg-brand-500 hover:bg-brand-600 text-white font-display font-semibold text-[12.5px] transition-colors">ყველა ექსპერტი <Icon.arrow className="w-3.5 h-3.5" /></Link>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 gap-3">
              {experts.map(e => (
                <Link key={e.id} href={`/tutors/${e.id}`} className="group rounded-card border border-ink-200 bg-white hover:border-ink-300 hover:shadow-card p-4 flex gap-3.5 transition-all">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5 min-w-0">
                      ...
                      <span className="inline-flex items-center justify-center w-3.5 h-3.5 rounded-full bg-brand-500 text-white shrink-0"><Icon.check className="w-2 h-2" /></span>
                    </div>
          ...
          <div className="space-y-2">
            {framing.related.map((r, i) => (
              <button key={i} type="button" onClick={() => goAsk(r)} className="w-full text-left rounded-card border border-ink-200 bg-white hover:border-brand-300 hover:bg-brand-50/30 px-4 py-3 flex items-center justify-between gap-3 group transition-colors">
                ...
                <span className="w-6 h-6 rounded-full bg-ink-100 group-hover:bg-brand-500 group-hover:text-white text-ink-500 inline-flex items-center justify-center shrink-0 transition-colors"><Icon.arrow className="w-3.5 h-3.5" /></span>
              </button>
            ))}
          </div>
        </section>
