...
type Item = {
   ...
   *  shared `primaryPriceLabel`, so this page can't disagree with /tutors. */
  priceLabel: string
  ...
  return (
    <div>
                ...
                <tr className="text-left text-white/40 font-display uppercase text-micro">
                  ...
                  {items.map(t => (
                    <th key={t.id} className="font-medium py-2 px-2">
                      <Link href={`/tutors/${t.tutorId}`} className="inline-flex items-center gap-1.5 text-white normal-case tracking-normal hover:text-brand-300 transition-colors duration-fast">
      ...
      {items.map(t => (
        ...
        // and made this card structurally unlike /tutors, which the very same
        // saved experts are browsed on. Same reasoning as the browse card's own
        // banner→thumb move: see the long note at app/tutors/client.tsx.
        <div key={t.id} className="relative rounded-card border border-ink-200 bg-white group hover-lift flex flex-col p-4 sm:p-5">
          {/* Saved heart — filled/brand (it IS saved); warms to danger on
              ...
              CARD corner (the /tutors pattern) and the identity column reserves
          ...
          <Link href={`/tutors/${t.tutorId}`} className="flex items-start gap-3.5 min-w-0">
            <Image
            ...
            <div className="min-w-0 flex-1 pr-9">
              {/* ROLES SWAPPED to match /tutors: the CHIP carries our taxonomy,
          ...
          <div className="flex flex-col flex-1">
            ...
            <div className="mt-auto pt-4">
              {/* The FLAGSHIP tier's price and its REAL length. The duration is
                  ...
                  wasn't) gives way to the actual minutes — same sentence /tutors
