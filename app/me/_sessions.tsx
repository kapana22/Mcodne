...
const SessionRow = ({ s, onOpen, onCancel, cancelling }: { s: Session; onOpen: (s: Session) => void; onCancel?: (s: Session) => void; cancelling?: boolean }) => {
  ...
  <article onClick={() => onOpen(s)} className="cursor-pointer grid grid-cols-[auto_1fr] sm:grid-cols-[64px_auto_minmax(0,1fr)_auto_auto] gap-x-4 gap-y-4 sm:gap-x-8 items-center py-4 sm:py-5 px-5 sm:px-6 hover:bg-ink-50/40 transition-colors duration-fast">
    ...
    <div className="col-span-2 sm:col-span-1 flex items-center justify-end gap-1.5 shrink-0 flex-wrap" onClick={e => e.stopPropagation()}>
      {s.status === 'confirmed' && (
        <>
          <Link href={`/me/messages/${s.id}`} aria-label="მიმოწერა" className="hidden md:inline-flex h-10 sm:h-9 w-9 rounded-btn border border-ink-200 hover:border-ink-300 text-ink-600 items-center justify-center transition-colors duration-fast">
      ...
      {s.status === 'completed' && (
        <>
          {!s.reviewed && (
            <Link href={`/me/bookings/${s.id}?review=1`} className="h-10 sm:h-9 px-3.5 rounded-btn bg-brand-600 hover:bg-brand-700 text-white font-display font-semibold text-small tracking-wide inline-flex items-center gap-1.5 transition-colors duration-fast">
          ...
          <Link
            href={
              s.tutorId
                ? `/experts/${s.tutorId}?topic=${encodeURIComponent(s.topic)}&duration=${s.duration}&price=${s.price}&rebook=1`
                : '/experts'
            }
      ...
      {s.status === 'cancelled' && (
        <Link href="/experts" className="h-10 sm:h-9 px-3.5 rounded-btn bg-white border border-ink-200 hover:border-ink-300 text-ink-700 font-display font-medium text-meta tracking-wide transition-colors duration-fast inline-flex items-center">
          ხელახლა
        </Link>
      )}
    ...
    <section className="rounded-card border border-ink-200 bg-white overflow-hidden">
      ...
      <div className="px-5 sm:px-6 pt-5 border-b border-ink-100">
        <div className="flex items-baseline justify-between flex-wrap gap-3">
          ...
          <Link href="/me/bookings" className="font-display text-meta font-semibold text-brand-700 hover:text-brand-800 inline-flex items-center gap-1 min-h-[40px] sm:min-h-0">
            მთლიანი ისტორია
          </Link>
        </div>
      ...
      <div className="divide-y divide-ink-100">
        {loading ? (
          ...
          <EmptyState
            ...
            cta={tab === 'upcoming' ? { label: 'იპოვე ექსპერტი', href: '/experts' } : undefined}
          />
        ) : tab === 'upcoming' ? (
      ...
      {rows.length > 0 && (
        <div className="px-5 sm:px-6 py-3.5 bg-ink-50/40 border-t border-ink-100 flex items-center justify-between">
          ...
          <Link href="/me/bookings" className="font-display text-meta font-semibold text-ink-700 hover:text-ink-900 inline-flex items-center gap-1">
            სრული სიის ნახვა
