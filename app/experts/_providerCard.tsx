...
export const providerHref = (m: Pick<ProviderRow, 'slug'>) => (m.slug ? `/experts/${m.slug}` : null)
...
export function ProviderCard({ m, view = 'grid' }: { m: ProviderRow; view?: EntityView
 }) {
  const href = providerHref(m)
  const row = view === 'list'
  ...
  return (
    <EntityCard
      ...
      title={
        ...
        <div className="flex items-center gap-x-1.5 gap-y-1 min-w-0 flex-wrap">
          ...
          {/* ⚠️ „გადამოწმებული", NEVER „ვერიფიცირებული" — the lexicon's rule,
              and the badge an admin actually ticks (ServiceProfile.verified).
              Nothing else is labelled: a badge on every card distinguishes
              nobody, which is why „ინდივიდუალური" is not printed either. */}
          {m.verified && (
      ...
      /* The bio, or the one crafted line when there is no bio — a migrated
         professional wrote a `headline` and may never have written an `about`,
         and the slot is the same slot. */
      about={m.about ?? m.headline}
