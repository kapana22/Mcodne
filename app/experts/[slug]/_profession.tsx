...
// The PROFESSION landing — /experts/<profession> (was /konsultacia/[slug] until
// stage 8, 2026-08-19; the middleware 308s the old address). A PART of the
// /experts/[slug] route: page.tsx resolves the slug against lib/professionSeo
// FIRST and renders this; anything else is an expert profile.
//
// Targets the „<პროფესია>-თან კონსულტაცია" head terms, which are a separate
// search intent from the sphere terms (see the long note at the top of
// lib/professionSeo.ts). Content is static and lives in code; only the expert
// grid touches the DB.
//
// NO generateStaticParams for this route, deliberately. Adding it flipped the
// route to SSG in the build output even alongside `dynamic = 'force-dynamic'` —
// which would bake the expert grid at BUILD time, and Railway's build container
// can't reach the DB (same trap documented in app/sitemap.ts). Every page would
// have shipped a permanently empty expert list. The slug set is validated at
// request time against professionBySlug instead.

const SITE_URL = (process.env.NEXT_PUBLIC_SITE_URL || 'https://mcodne.ge').replace(/\/$/, '')

/** The public address of a profession landing. */
export const professionPath = (p: Pick<ProfessionSeo, 'slug'>) => `/experts/${p.slug}`

export function professionMetadata(p: ProfessionSeo): Metadata {
  return {
    ...
    alternates: { canonical: `${SITE_URL}${professionPath(p)}` },
    ...socialMeta({
      title: `${p.keyword} | მცოდნე`,
      description: p.metaDescription,
      url: professionPath(p),
    }),
  }
}
...
export async function ProfessionLanding({ p }: { p: ProfessionSeo }) {
  ...
  const serviceLd = {
    ...
    availableChannel: {
      '@type': 'ServiceChannel',
      serviceUrl: `${SITE_URL}${professionPath(p)}`,
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* size="content" — every section here is capped ~760 too, so a 1280
          shell only produced an empty right-hand column. */}
      {/* The standard section rhythm, matching the /experts hub — a landing
          page and its index reading at two different rhythms is drift, not
          design. */}
      <Container as="main" size="content" className="py-12 lg:py-16">
          ...
          {experts.length === 0 ? (
            <div className="max-w-[560px]">
              <EmptyState
                ...
                description="ბაზა თანდათან ივსება. სანამ ელოდები, გადახედე სხვა კატეგორიების ექსპერტებს."
                cta={{ label: 'ნახე ყველა ექსპერტი', href: '/experts' }}
              />
              <div className="mt-3 text-center">
                <Link href="/join" className="inline-flex items-center gap-1.5 text-small font-display font-semibold text-brand-700 hover:text-brand-800 transition-colors duration-fast">
                    ...
                    <div className="mt-4 pt-3 border-t border-ink-100 flex items-center justify-between text-meta">
                      <span className="inline-flex items-center gap-1 text-ink-600">
                        {/* An unrated expert renders NOTHING here — the same decision
                            app/experts/_card.tsx took on 2026-07-31 and wrote down:
                            ...
                            on /experts — and THIS is the page a stranger lands on
                            from Google. */}
        ...
        {/* Cross-link to the sphere — the catalogue filtered to it, since
            /categories/<slug> was retired in stage 8 (the middleware 308s it
            to exactly this address). GUARDED on there being experts
            (2026-08-11): `experts.length > 0` is the honest proxy that the
            sphere has somebody in it, which is the only case where „see the
            whole sphere" leads anywhere worth going. */}
        <div className="mt-12 flex flex-wrap items-center gap-x-6 gap-y-2">
          {experts.length > 0 && (
            <Link href={`/experts?category=${p.categorySlug}`} className="tap-area inline-flex items-center gap-1.5 text-body font-semibold text-brand-700 hover:underline">
              {sphere.keyword} <Icon.arrow className="w-3.5 h-3.5" />
            </Link>
          )}
          <Link href="/experts" className="tap-area inline-flex items-center gap-1.5 text-body font-semibold text-ink-700 hover:underline">
            ყველა ექსპერტი
          </Link>
        </div>
        ...
        <section className="mt-16">
          <Eyebrow className="mb-4">სხვა ექსპერტები</Eyebrow>
          <ul className="flex flex-wrap gap-x-5 gap-y-2">
            {professions.filter(o => o.slug !== p.slug).map(o => (
              <li key={o.slug}>
                <Link href={professionPath(o)} className="text-small text-ink-600 hover:text-brand-700 transition-colors duration-fast">
                  {o.keyword}
                </Link>
              </li>
            ))}
