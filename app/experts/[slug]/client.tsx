'use client'
// /experts/[slug] — the expert profile container. Owns the fetch, the booking
...
// used by the /experts listing. The slot math, tz labels and defaults that used
...
/* Calendar / DayTimeline / Steps / intake / payment / OrderSummary /
   ...
   the shared implementation both this profile and /experts render. */
...
/* ───── In-page anchor nav (DESIGN_FIX_PROMPT 1.6) ─────
   ...
   /experts/[slug] as a focused screen), so the mobile offset only has to clear
...
export default function ExpertProfilePage({ initialTutor, initialUser, masterHref, requestHref = null }: { initialTutor?: any; initialUser?: PublicMe | null; masterHref?: string | null; requestHref?: string | null }) {
  return (
    <Suspense fallback={<div className="min-h-screen bg-white" />}>
      <ExpertProfile initialTutor={initialTutor ?? null} initialUser={initialUser} masterHref={masterHref ?? null} requestHref={requestHref} />
    </Suspense>
  )
}

// `masterHref` — the same person's second profile (/experts/<slug> since stage 11) when they
// have a published one; resolved server-side (page.tsx), rendered as one link.
function ExpertProfile({ initialTutor, initialUser, masterHref, requestHref }: { initialTutor: any; initialUser?: PublicMe | null; masterHref: string | null; /** ⚠️ `/request?to=<slug>` when the subsystem exists, null otherwise — gated ONCE in page.tsx. The rail turns it into its SECONDARY action; there is no third button. */ requestHref: string | null }) {
  const params = useParams<{ slug: string }>()
  const searchParams = useSearchParams()
  // The REAL profile id — never the URL segment.
  //
  // The URL is now the human slug („/experts/ana-gagoshidze"), and every request
  // this component makes (/api/tutors/…, /api/favorites?tutorId=…, booking
  // submits) is keyed by the profile's actual id. Reading `params.slug` here sent
  ...
  const tutorId = initialTutor?.id ?? params?.slug
  ...
  useEffect(() => {
    ...
    // ⚠️ `/api/tutors/…` AND NOT `/api/experts/…` (fixed 2026-08-19). The page
    // moved to /experts; the API route did not, and renaming the fetch without
    // it made this 404 on every profile — the page still rendered from the
    // server seed, so nothing looked broken while the client refresh, the
    // services, the availability and the reviews silently stopped arriving.
    // Three other callers (BookingFlow ×2, RescheduleTimePicker) always used
    // the real path; this one was the odd one out.
  ...
  // pattern as app/experts/client.tsx. It's a prop, identical on the server and
  ...
  // /api/experts/[slug] JSON this page already loaded (no extra request).
  ...
  const onMessageExpert = () => {
    ...
    router.push(`/me/messages/u/${expertUserId}`)
  }
      ...
      <div className="font-sans bg-white text-ink-900 antialiased min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-16">
          <div className="max-w-[480px] w-full text-center">
            ...
            <Link href="/experts" className="tap-shrink mt-6 inline-flex h-11 px-5 rounded-btn bg-brand-600 hover:bg-brand-700 text-white font-display font-semibold text-body items-center gap-2">
              ყველა ექსპერტი
            </Link>
          </div>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-16">
          <div className="max-w-[480px] w-full text-center">
            ...
            <div className="mt-6 flex items-center justify-center gap-2">
              ...
              <Link href="/experts" className="h-11 px-5 rounded-btn border border-ink-200 hover:border-ink-300 hover:bg-ink-50 text-ink-800 font-display font-semibold text-small tracking-wide inline-flex items-center transition-colors duration-fast">
                ყველა ექსპერტი
              </Link>
            </div>
          </div>
    ...
    <div className="font-sans bg-white text-ink-900 antialiased">
      ...
      <Container as="main" id="main" className="pt-4 sm:pt-7 lg:pt-9 pb-[168px] lg:pb-16">
        ...
        <div className="sm:mt-6 grid lg:grid-cols-[1fr_360px] gap-8 xl:gap-12">
          ...
          <div className="min-w-0">
            <VideoHero tutorId={tutorId} tutor={tutorData} requireAuth={requireAuth} viewerCantFav={viewerCantFav} masterHref={masterHref} />
