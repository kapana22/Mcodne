'use client'
// /experts/[slug] — the top of the profile: breadcrumb + the video hero.
...
export const Breadcrumb = ({ tutor }: { tutor: TutorDetail | null }) => (
  <nav aria-label="ნავიგაცია" className="font-display text-micro font-semibold uppercase flex items-center gap-2 text-ink-500">
    <Link href="/experts" className="hover:text-ink-900 transition-colors duration-fast">ექსპერტები</Link>
    {tutor?.category && (
      <>
        <Icon.chevR className="w-3 h-3 text-ink-300" />
        {/* The catalogue filtered to the sphere — /categories/* was retired in
            stage 8 (2026-08-19), and lib/categoryRoutes is still the one place
            that says where a category lives, so this goes through it. */}
...
/* priceForDuration + TUTOR_DEFAULTS moved to components/booking/slots.ts —
   the old "MUST stay identical to app/experts/page.tsx" twin blocks are now a
   single import on both surfaces. Covered by tests/tutor-mapping.test.ts. */

/* ───── Video hero — cover moment with overlapping avatar ───── */
export const VideoHero = ({ tutorId, tutor, requireAuth, viewerCantFav = false, masterHref = null }: { tutorId?: string; tutor: TutorDetail | null; requireAuth?: () => boolean; viewerCantFav?: boolean; masterHref?: string | null }) => {
  ...
  const shareTutorLink = async () => {
    ...
    const url = `${origin}/experts/${tutorId}`
  ...
  // bounces to /signin?redirect=/experts/{id}?fav=1, and on return we re-apply the
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
          ...
          window.location.href = `/signin?redirect=${encodeURIComponent(`/experts/${tutorId}?fav=1`)}`
          return
        }
  ...
  // blobs never reach the client — the SSR seed and /api/experts/[slug] both null
      ...
      <div className={`relative rounded-card bg-white border border-ink-200 shadow-card px-5 sm:px-7 pt-5 sm:pt-6 pb-6 ${hasVideo ? 'mt-4' : ''}`}>
        ...
        <div className="mt-3 sm:mt-4">
          ...
          {(() => {
            ...
            if (!li && !web && !masterHref) return null
            return (
              <div className="mt-4 flex flex-wrap items-center gap-2">
                {/* Dual provider: the same person's second profile. ONE address
                    space since stage 11 — it is /experts/<slug> too, resolved by
