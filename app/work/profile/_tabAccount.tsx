    ...
    <>
        ...
        <form onSubmit={saveName} className="p-6 rounded-card border border-ink-200 bg-white space-y-4">
          <Field label="სახელი და გვარი">
            ...
            <p className="mt-1.5 text-meta text-ink-500 leading-snug">შენი სახელი — ასე გამოჩნდები კლიენტებთან.</p>
          </Field>
              ...
              <div className="min-w-0">
                ...
                <p className="text-meta text-ink-500 mt-1 leading-snug max-w-[480px]">
                  {profile.available === false
                    ? 'ძებნაში აღარ ჩანხარ. მიმდინარე სამუშაოები გრძელდება; ახალ მოთხოვნებს ვეღარ მიიღებ.'
                    : 'ჩანხარ ძებნის სიაში. გამორთე დროებითი შესვენებისთვის — მიმდინარე სამუშაოები არ დაზარალდება.'}
                </p>
              </div>
              <button type="button"
                onClick={async () => {
                  ...
                  try {
                    // ⚠️ `/api/me/tutor` UNTIL 2026-08-26 — a 404 since the
                    // consultation profile went, so this switch could not
                    // pause or re-list a profile at all: it flipped, failed,
                    // and reverted with „შენახვა ვერ მოხერხდა". Same body,
                    // `{ available }`, on the route that owns the column now.
