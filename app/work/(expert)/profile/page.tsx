...
import {
  PWD_MIN, PWD_MIN_MSG,
  type Category, type CertForm, type Certificate,
  type EduForm, type Education, type ExpForm, type Experience, type Me,
...
export default function TutorProfilePage() {
  ...
  useEffect(() => {
    ...
    const SECTION_TO_TAB: Record<string, number> = {
      ...
      'section-certificates': 1, 'section-education': 1, 'section-experience': 1,
  ...
  const loadCredentials = async () => {
    try {
      const [c, e, x, av] = await Promise.all([
  ...
  return (
    <div>
        <PageHeader
          ...
          actions={profile && (
            <div className="flex items-center gap-2">
              {/* ⚠️ THE WAY TO WHAT LEFT THIS PAGE. The „სესიები" tab is
                  /work/services now, and until the rail carries that item this
                  is the only trace of the move a returning expert would see —
                  the tab is simply gone, with nothing saying where to. */}
              <a
                href="/work/services"
          ...
          <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-8 lg:items-start">
            ...
            <div className="min-w-0">
              ...
              <div className="flex border-b border-ink-200 mb-6 overflow-x-auto scrollbar-hide rail-fade-end" role="tablist" aria-label="პროფილის სექციები">
                {['პროფილი', 'კვალიფიკაცია', 'ანგარიში'].map((label, i) => {
        ...
        />
            ...
            {/* ——— Tab 1: კვალიფიკაცია — certificates + education + experience.
                Each tab is one clear theme (this was crammed into the old
                „სერვისები და კვალიფიკაცია“, and what an expert SELLS left this
                page altogether for /work/services). ——— */}
            {profile && (
        ...
        />
            ...
            {/* ——— Tab 2: ანგარიში (visibility + response-time + password) ——— */}
