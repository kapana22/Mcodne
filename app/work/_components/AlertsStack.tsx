    ...
    if (awaitsRescheduleAnswer(b)) {
      alerts.push({
        ...
        sub: b.rescheduleRequest?.newStartAt
          ...
          : undefined,
        cta: { label: 'პასუხი', href: `/work/bookings/${b.id}` },
      })
    }
  }
  ...
  for (const b of bookings) {
    if (awaitsClosure(b, now)) {
      alerts.push({
        ...
        cta: { label: 'დახურვა', href: `/work/bookings/${b.id}` },
      })
    }
  }
    ...
    <section aria-label="საჭიროებს ყურადღებას" className="mb-6 space-y-2">
      ...
      {rest > 0 && (
        <div className="text-right">
          <Link href="/work/jobs?tab=attention" className="text-small text-brand-700 hover:text-brand-800 font-display font-semibold">
            კიდევ {rest} საკითხი
          </Link>
        </div>
      )}
