...
// consultations, no calendar. See prisma/schema → ProviderApplication for why
// this is a separate table rather than a flag.
//
// ⚠️ THE VALIDATION IS NOT WRITTEN HERE. `ProviderApplicationInput` lives in
// lib/providerApplication and the FORM imports the same object. That is the rule
...
export async function GET() {
  ...
  const counts = row ? await prisma.$queryRaw<{ has: boolean; n: number }[]>`
    ...
    FROM "ProviderApplication" WHERE "userId" = ${user.id}
  ` : []
...
export async function POST(req: Request) {
  ...
  const data = {
    ...
    // see ProviderApplicationInput's two rules for priceList.
    priceList: d.priceList,
    photoUrl: d.photoUrl,
    workPhotos: d.workPhotos,
  }

  const app = await prisma.providerApplication.upsert({
    where: { userId: user.id },
  ...
  after(async () => {
    ...
    try {
      await notify(user.id, {
        ...
        body: PROVIDER_STATUS_TEXT.SUBMITTED,
        href: '/join?can=WORK',
      })
    } catch {}
    ...
    try {
      ...
      await notifyMany(admins.map(a => a.id), {
        ...
        body: `${d.fullName} · ${PROVIDER_KIND_LABEL[d.kind]} · ${d.services.map(topicLabel).slice(0, 3).join(', ')}`,
        href: '/admin#masters',
      })
