...
import {
  masterApprovedEmail, masterRevisionEmail, masterRejectedEmail,
} from '@/lib/emailTemplates'
import { audit } from '@/lib/audit'
import { providersOn, PROVIDER_ROUTE } from '@/lib/requests'
import { approvalBlockers } from '@/lib/masterApplication'
 ...
 * here rather than passed in — `MasterApplication` carries its own name and
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const app = await prisma.masterApplication.findUnique({ where: { id } })
  if (!app) return notFound()
  ...
  if (action !== 'approve') {
    ...
    const { count } = await prisma.masterApplication.updateMany({
    ...
    after(() => sendMasterMail(app.userId, status === 'NEEDS_REVISION'
      ? masterRevisionEmail({ name: app.fullName, note })
      : masterRejectedEmail({ name: app.fullName, note })))
    await audit(me.id, `master.${action}`, { targetType: 'MasterApplication', targetId: id })
    return NextResponse.json({ ok: true, status })
  }

  /* ── approve ─────────────────────────────────────────────────────────── */

  // The soft gate closing. lib/masterApplication → approvalBlockers explains why
  ...
  await prisma.$transaction(async tx => {
    const { count } = await tx.masterApplication.updateMany({
      where: { id, status: { not: 'APPROVED' } },
  ...
  try {
    ...
    if (profile) await ensureMasterSlug(profile.id)
  ...
  after(() => sendMasterMail(app.userId, masterApprovedEmail({ name: app.fullName, note: note || null })))
  await audit(me.id, 'master.approve', {
    targetType: 'MasterApplication',
    targetId: id,
