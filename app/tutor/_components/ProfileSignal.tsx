...
export function diagnose(o: {
  ...
  if (views === 0) {
    return {
      key: 'no-views',
      text: `ბოლო ${days} დღეში პროფილი არავის უნახავს. ეს ხილვადობის და არა დარწმუნების საკითხია — შეამოწმე, სფერო და სათაური ზუსტად აღწერს თუ არა შენს საქმეს.`,
      cta: { label: 'პროფილის შემოწმება', href: '/tutor/profile' },
    }
  }
  ...
  if (bookings === 0) {
    return {
      ...
      cta: { label: 'პროფილის რედაქტირება', href: '/tutor/profile' },
    }
  }
  ...
  return {
    key: 'working',
    text: `${views} ნახვიდან ${bookings} ჯავშანი (${pct(views, bookings)}%). პროფილი მუშაობს — შემდეგი ბერკეტი გრაფიკია: რაც მეტი თავისუფალი დროა, მით მეტი ასარჩევი აქვს სტუდენტს.`,
