...
export function ImpersonationBanner() {
  ...
  return (
    ...
    // so two elements claimed the same pinned strip and the banner (z-impersonate)
    ...
    // pins to the viewport top exactly as it does for everyone else. `z-impersonate`
    // is kept so the banner still stacks above page content while visible.
    <div className="relative z-impersonate bg-warning-100 border-b border-warning-300 text-warning-900 px-4 sm:px-6 py-2 flex items-center justify-between gap-3 text-small font-display font-semibold">
      ...
      <button
        ...
        onClick={exit}
        disabled={busy}
        className="h-8 px-3 rounded-btn bg-warning-900 hover:bg-ink-900 disabled:opacity-60 text-white font-display font-semibold text-meta inline-flex items-center gap-1 shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-warning-100"
      >
        {busy ? '…' : 'გამოსვლა'}
      </button>
    </div>
