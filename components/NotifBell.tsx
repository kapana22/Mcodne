...
export function NotifBell() {
  ...
  // The panel is a menu of actions — give it real menu keys (↑/↓/Home/End/
  // Escape) instead of making people Tab through every notification.
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-expanded={open}
        aria-haspopup="menu"
        className="relative w-10 h-10 rounded-btn text-ink-600 hover:text-ink-900 hover:bg-ink-100 inline-flex items-center justify-center transition-colors duration-fast"
      >
      ...
      {open && (
        ...
        <div
          ...
          className="fixed inset-x-3 top-16 sm:absolute sm:inset-x-auto sm:right-0 sm:top-full sm:mt-2 sm:w-[340px] bg-white border border-ink-200 rounded-card shadow-float z-50 overflow-hidden motion-safe:animate-scale-in origin-top sm:origin-top-right"
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-ink-100">
            ...
            <button
              ...
              className="text-meta font-display font-semibold text-brand-700 hover:text-brand-800 disabled:text-ink-500 disabled:cursor-default transition-colors duration-fast"
            >
              ყველა წაკითხულად
            </button>
          </div>
          <div className="max-h-[420px] overflow-y-auto">
            {items.length === 0 ? (
              <div className="py-10 text-center text-small text-ink-500">შემოსული ცარიელია</div>
            ) : (
                  ...
                  return (
                    <li key={n.id}>
                      ...
                      <button
                        ...
                        role="menuitem"
                        onClick={() => clickItem(n)}
                        className={`no-caps relative w-full text-left px-4 py-3 hover:bg-ink-50 transition-colors duration-fast flex items-start gap-2.5 ${isUnread ? 'bg-brand-50/40' : ''}`}
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-baseline justify-between gap-2">
                            <span className={`font-display text-small truncate ${isUnread ? 'font-bold text-ink-900' : 'font-medium text-ink-700'}`}>
                              {n.title}
                            </span>
                            <span className="text-meta text-ink-400 font-mono tabular-nums shrink-0">{timeAgo(n.createdAt)}</span>
                          </div>
                          {n.body && (
                            <div className="text-meta text-ink-600 mt-0.5 line-clamp-2">{n.body}</div>
                          )}
                        </div>
                      </button>
                    </li>
          ...
          <div className="px-4 py-2.5 border-t border-ink-100 bg-ink-50/40">
            <Link
              ...
              className="w-full h-9 rounded-btn font-display text-meta font-semibold text-ink-700 hover:bg-white inline-flex items-center justify-center gap-1.5 transition-colors duration-fast"
            >
              ყველა შეტყობინება
            </Link>
          </div>
